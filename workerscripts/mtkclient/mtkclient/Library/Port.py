            ...
            while i < length:
                if ep_out(int.to_bytes(startcmd[i], 1, 'little')):
                    ...
                    if len(v) == 1 and v[0] == ~(startcmd[i]) & 0xFF:
                        ...
                        i = 0
            self.info("A MTK device has been detected :)")
            return True
                ...
                if self.cdc.connected and self.run_serial_handshake():
                    ...
                    if loop == 5:
                        ...
                        self.info("Hint:\n\nPower off the device before connecting.\n" +
                                  "For Download mode, press and hold vol up, vol dwn, or all hw buttons and " +
                                  "connect the usb cable (usually the recovery button).\n" +
                                  "For preloader mode, don't press any hw button and connect the usb cable.\n"
                                  "If it is already connected and on, hold power until the screen turns black to reset.\n")
                        sys.stdout.write('\n')
                    if loop >= 10:
                        sys.stdout.write('.')
                    if loop >= 20:
...
# Credit to mtkclient for the original code
# Modified a bit by mpax235
