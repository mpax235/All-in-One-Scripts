...
class Preloader(metaclass=LogBase):
    ...
    def init(self, maxtries=None, display=True):
        ...
        self.info("Status: Waiting for the Preloader, please reconnect the device to Download Mode")
        ...
        if self.display:
            self.info("\tSoC:\t\t\t" + cpu + "(" + self.config.chipconfig.description + ")")
    ...
    def get_blver(self):
        if self.usbwrite(self.Cmd.GET_BL_VER.value):
            ...
            if res == self.Cmd.GET_BL_VER.value:
                # We are in boot rom ...
                self.info("Download mode detected.")
